...
export default async function Supplier() {
  const [offers, trades, agreements] = await Promise.all([
    productionGet<{ items: Offer[] }>("/v1/offers"),
    productionGet<{ items: Trade[] }>("/v1/trades"),
    productionGet<{ items: BoundAgreement[] }>("/v1/agreements"),
  ]);
  if (offers.state !== "CONNECTED")
    return (
      <Shell current="/supplier">
        <ProductionBoundary state={offers.state} reason={offers.reason} />
      </Shell>
    );
  ...
  return (
    <Shell current="/supplier">
      <DocketHeader
        title="Supplier inventory register"
        refId="LIVE-SUPPLY"
        rails="Server enforced"
      />
      <section className="two-col">
        <div className="sheet">
          <h2>Current verified offers</h2>
          {offers.data.items.length ? (
            offers.data.items.map((offer) => (
              <article
                className="register-entry"
                key={`${offer.id}-${offer.product_family}`}
              >
                <FieldTable
                  rows={[
                    ["Material", offer.product_family],
                    ["Available", `${offer.quantity_mt} MT`],
                    ["MOQ", `${offer.moq_mt} MT`],
                    [
                      "Supplier net",
                      `${offer.currency} ${offer.supplier_net}/kg`,
                    ],
                    [
                      "Freshness",
                      offer.freshness,
                      <Status
                        key="f"
                        state={
                          offer.freshness === "CURRENT" ? "pass" : "unknown"
                        }
                      />,
                    ],
                  ]}
            ...
            <EmptyRegister
              title="No executable offer"
              body="Send current inventory from the verified supplier mailbox. It remains a proposal until registration and documents pass."
            />
          )}
        </div>
        <div className="sheet">
          ...
          {trades.data.items.length ? (
            ...
            <EmptyRegister
              ...
              body="Buyer identity remains unavailable until a compatible match, both protected acceptances, and an acknowledged fee lock exist."
            />
          )}
        </div>
      </section>
