...
def test_fee_lock_and_identity_release_choke_point():
    ...
    for c in ("domestic_route=cashfree","fee_lock=true","identity_release=after_lock","mutations=8","instruction_created=not_locked","no_rail=reject","bank_draft=not_locked","replay=idempotent"): assert c in r.stdout
