...
def test_provider_payloads_webhooks_and_entitlement_gate():
    ...
    for claim in ("escrow=separate_broker_item","escrow_webhook=fetch_confirmed","cashfree=order_capture_then_supplier_split","cashfree_commission=merchant_retained","razorpay=order_capture_then_integer_paise_transfer","webhooks=provider_specific","instruction_created=not_fee_locked"): assert claim in result.stdout
