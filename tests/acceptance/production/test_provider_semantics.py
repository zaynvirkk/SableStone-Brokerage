...
def test_provider_payloads_webhooks_and_entitlement_gate():
    ...
    for claim in ("escrow=separate_broker_item","escrow_webhook=fetch_confirmed","provider_beneficiaries=verified_accounts","provider_allocations=exact","promotion=current_atomic","cashfree=order_capture_then_supplier_split","cashfree_reconciliation=exact","cashfree_commission=merchant_retained","razorpay=order_capture_then_integer_paise_transfer","webhooks=provider_specific","instruction_created=not_fee_locked"): assert claim in result.stdout
    ...
    assert "assertProviderEntitlementEvidence" in projection
    assert "assertEntitlementPromotionWindow" in projection
    assert "resolveBoundCurrent" in projection
    assert "provider party mapping changed before fee lock" in projection
    assert "pa.environment='PRODUCTION' and ar.authority_kind='PROVIDER_WRITTEN_APPROVAL'" in projection
...
def test_provider_party_references_are_verified_encrypted_and_bound():
    ...
    assert "provider entitlement beneficiary and allocation paths required" in factory
    assert "ar.authority_kind='PROVIDER_WRITTEN_APPROVAL'" in factory
    assert "ar.authority_kind='PROVIDER_WRITTEN_APPROVAL'" in resolver
