...
type Scope = "GLOBAL" | "COUNTRY" | "POLYMER" | "APPLICATION" | "BUYER" | "SUPPLIER" | "PAIR";
...
const PRIOR_STRENGTH: Readonly<Record<Scope, number>> = Object.freeze({ GLOBAL: 20, COUNTRY: 15, POLYMER: 12, APPLICATION: 10, BUYER: 8, SUPPLIER: 8, PAIR: 6 });
...
function posterior(previous: Prior, row: QueryResultRow, scope: Scope): Prior {
  ...
  return {
    ...
    days: settled > 0 ? String(row.days_to_cash) : previous.days,
...
export async function refreshMatchPriority(pool: Pool | PoolClient, matchId: string): Promise<void> {
  const facts = (await pool.query(
    "select m.id,o.supplier_id,d.buyer_id,o.product_family,coalesce(d.product_spec->>'application',o.product_spec->>'application','UNKNOWN') application,d.quantity_mt,d.standing,coalesce(fe.realized_commission_per_kg,pd.commission_per_kg) expected_commission_per_kg,case when sj.country_code='IN' and bj.country_code='IN' then 'DOMESTIC_INDIA' else 'INTERNATIONAL' end geography from matches m join supplier_offers o on o.id=m.offer_id and o.version=m.offer_version join buyer_demands d on d.id=m.demand_id and d.version=m.demand_version join pricing_decisions pd on pd.match_id=m.id and pd.state='EXECUTABLE' left join final_economics_snapshots fe on fe.match_id=m.id join organization_jurisdictions sj on sj.organization_id=o.supplier_id and sj.state='VERIFIED' and sj.valid_until>now() join organization_jurisdictions bj on bj.organization_id=d.buyer_id and bj.state='VERIFIED' and bj.valid_until>now() where m.id=$1", [matchId],
  )).rows[0];
  if (!facts) return;
  const stats = (await pool.query(
    "with history as(select hm.id match_id,ho.supplier_id,hd.buyer_id,ho.product_family,coalesce(hd.product_spec->>'application',ho.product_spec->>'application','UNKNOWN') application,case when hsj.country_code='IN' and hbj.country_code='IN' then 'DOMESTIC_INDIA' else 'INTERNATIONAL' end geography,ht.id trade_id,ht.state,ht.created_at,ht.updated_at,fm.fulfilled_quantity_mt,fm.contracted_quantity_mt from matches hm join supplier_offers ho on ho.id=hm.offer_id and ho.version=hm.offer_version join buyer_demands hd on hd.id=hm.demand_id and hd.version=hm.demand_version join organization_jurisdictions hsj on hsj.organization_id=ho.supplier_id and hsj.state='VERIFIED' join organization_jurisdictions hbj on hbj.organization_id=hd.buyer_id and hbj.state='VERIFIED' left join trades ht on ht.match_id=hm.id left join fulfillment_measurements fm on fm.trade_id=ht.id where hm.compatible and hm.id<>$1),scopes(scope,rank) as(values('GLOBAL',1),('COUNTRY',2),('POLYMER',3),('APPLICATION',4),('BUYER',5),('SUPPLIER',6),('PAIR',7)) select s.scope,count(distinct h.match_id)::int opportunity_count,count(distinct h.trade_id) filter(where h.state in('FUNDED','DISPATCHED','IN_TRANSIT','DELIVERED','ACCEPTED','SETTLED','RECURRING'))::int funded_count,count(distinct h.trade_id) filter(where h.state in('SETTLED','RECURRING'))::int settled_count,count(distinct h.trade_id) filter(where h.fulfilled_quantity_mt is not null)::int measured_count,coalesce(sum(round(1000*h.fulfilled_quantity_mt/nullif(h.contracted_quantity_mt,0))) filter(where h.fulfilled_quantity_mt is not null),0)::int fulfilled_milli_ratio_sum,coalesce(avg(extract(epoch from(h.updated_at-h.created_at))/86400) filter(where h.state in('SETTLED','RECURRING')),30)::text days_to_cash from scopes s join history h on case s.scope when 'GLOBAL' then true when 'COUNTRY' then h.geography=$2 when 'POLYMER' then h.geography=$2 and h.product_family=$3 when 'APPLICATION' then h.geography=$2 and h.product_family=$3 and h.application=$4 when 'BUYER' then h.buyer_id=$5 when 'SUPPLIER' then h.supplier_id=$6 when 'PAIR' then h.buyer_id=$5 and h.supplier_id=$6 end group by s.scope,s.rank order by s.rank", [matchId, facts.geography, facts.product_family, facts.application, facts.buyer_id, facts.supplier_id],
  )).rows;
  ...
  const evidenceState = estimate.observations > 0 ? "CALIBRATED" : "HEURISTIC",
    result = expectedProfitPriority({
      ...
      expectedMonths: known(decimal(facts.standing ? "24" : "1"), `match:${matchId}:term`),
