...
export interface WorkflowReceipt{readonly receiptId:string;readonly digest:string;readonly state:"ACCEPTED"|"REJECTED"|"UNKNOWN";readonly facts?:Readonly<Record<string,unknown>>;}
export interface BrokerageActivities{
 ...
 openDispute(input:{tradeId:string;disputeRequestId:string}):Promise<WorkflowReceipt>;
...
export async function DisputeWorkflow(input:{tradeId:string;disputeRequestId:string}):Promise<WorkflowReceipt>{return activities.openDispute(input)}
export async function RecurringDemandWorkflow(input:{tradeId:string}):Promise<readonly WorkflowReceipt[]>{let reconciled:WorkflowReceipt|undefined;for(let attempt=0;attempt<30*24;attempt++){const result=await activities.reconcile(input);if(result.state==="REJECTED")throw new Error("reconciliation mismatch");if(result.state==="ACCEPTED"){reconciled=result;break}await sleep("1 hour")}if(!reconciled)throw new Error("reconciliation expired");const schedule=await activities.recurrenceSchedule(input);if(schedule.state!=="ACCEPTED")return Object.freeze([reconciled,schedule]);const nextRequiredAt=String(schedule.facts?.nextRequiredAt??""),validUntil=String(schedule.facts?.validUntil??"");if(!nextRequiredAt||!validUntil||Number.isNaN(Date.parse(nextRequiredAt))||Number.isNaN(Date.parse(validUntil)))throw new Error("recurrence schedule unavailable");const delay=Math.max(Date.parse(nextRequiredAt)-Date.now(),0);if(delay>0)await sleep(delay);while(Date.now()<Date.parse(validUntil)){const recurring=await activities.recur(input);if(recurring.state==="REJECTED")throw new Error("recurrence rejected");if(recurring.state==="ACCEPTED")return Object.freeze([reconciled,recurring]);await sleep("1 day")}throw new Error("standing authorization expired without executable recurrence")}
