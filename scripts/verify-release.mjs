...
const {signatureAlgorithm,signatureBase64,...payload}=att,publicKeyPem=readFileSync("release-trust/sablestone-build.pub.pem","utf8"),trustRootSha256=createHash("sha256").update(publicKeyPem).digest("hex");
if(att.trustRootSha256!==trustRootSha256)throw new Error("release trust root mismatch");
